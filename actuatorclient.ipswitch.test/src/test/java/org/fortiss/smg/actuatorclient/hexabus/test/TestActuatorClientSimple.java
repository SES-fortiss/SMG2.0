...
public class TestActuatorClientSimple extends AbstractClient {
	...
	private static org.fortiss.smg.actuatorclient.hexabus.test.MockOtherBundles mocker;
...
*/
	...
	public void setUp() {
        ...
        if (master4config != null) {
            try {
                configList = master4config.getWrapperConfig("hexabus");
            } catch (TimeoutException e) {
                logger.error("ActuatorClient: Unable to connect to master (Timeout).");
            } finally {
                try {
            ...
            if (configList.size() > 0) {
                ...
                logger.info("Hexabus Wrapper started");
            } else {
            ...
            logger.debug("Hexabus Wrapper could not read config from Master");
        }
		try {
